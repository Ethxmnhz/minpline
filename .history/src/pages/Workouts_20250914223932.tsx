...
export const Workouts: React.FC = () => {
  ...
  const iso = new Date(today).toISOString();
  const safeKey = iso.replace(/\..+Z$/, 'Z').replace(/\./g,'_');
  const record = workoutSets[safeKey] || workoutSets[iso] || { pushups:0, pullups:0, legs:0 };
