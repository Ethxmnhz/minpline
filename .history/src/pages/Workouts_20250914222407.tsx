            ...
            <li key={w.id} style={{display:'flex', flexDirection:'column', gap:4}}>
              {editingId === w.id ? (
                <div className="card" style={{margin:0, background:'#1d2730'}}>
                  <input value={editDraft.type} onChange={e=>setEditDraft(d=>({...d,type:e.target.value}))} />
                  <input type="number" value={editDraft.calories||''} onChange={e=>setEditDraft(d=>({...d,calories:+e.target.value}))} />
