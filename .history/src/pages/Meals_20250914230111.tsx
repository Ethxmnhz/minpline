...
export const Meals: React.FC = () => {
  ...
  const cacheGet = useStore(s => s.getCachedNutrition);
  const cacheSet = useStore(s => s.cacheNutrition);
  const incCall = useStore(s => s.incNutritionCall);
  const incHit = useStore(s => s.incNutritionCacheHit);

  useEffect(() => {
    let active = true;
  const qRaw = foodQuery.trim();
  if (!qRaw) { setFoodResults([]); setRemoteResults([]); setRemoteError(''); return; }
  const q = qRaw.replace(/\\/g,' ').replace(/\s+/g,' ').trim();
  if (!q) { setFoodResults([]); setRemoteResults([]); setRemoteError(''); return; }
  ...
  const data = await fetchNutrition(q, { cacheGet, cacheSet, onCall: incCall, onHit: incHit });
      if (!active) return;
  ...
  const runRemoteSearch = async () => {
    ...
    setRemoteLoading(true); setRemoteError('');
  const data = await fetchNutrition(foodQuery.trim(), { cacheGet, cacheSet, onCall: incCall, onHit: incHit });
    if (!data.length) setRemoteError('No remote results');
