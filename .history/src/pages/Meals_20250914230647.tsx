...
export const Meals: React.FC = () => {
  ...
  useEffect(() => {
    let active = true;
    const qRaw = foodQuery.trim();
    if (!qRaw) { setFoodResults([]); setRemoteResults([]); if (remoteError) setRemoteError(''); return; }
    const q = qRaw.replace(/\\/g,' ').replace(/\s+/g,' ').trim();
    if (!q) { setFoodResults([]); setRemoteResults([]); if (remoteError) setRemoteError(''); return; }
    // always update local results
    setFoodResults(searchFoods(q));
    // handle remote gating
    if (!nutritionApiEnabled) {
      setRemoteResults([]);
      setRemoteLoading(false);
      if (q.length >= 2) setRemoteError('Remote search disabled (enable in Settings)'); else if (remoteError) setRemoteError('');
      return () => { active = false; };
    }
    const handle = setTimeout(async () => {
      if (!active) return;
      if (q.length < 2) { setRemoteResults([]); setRemoteError('Invalid query'); return; }
  ...
  const runRemoteSearch = async () => {
    if (!nutritionApiEnabled) return; // gated
  ...
  <input type="number" placeholder="Fats" value={form.fats||''} onChange={e=>setForm((f: MealInput)=>({...f,fats:+e.target.value}))} />
        <div style={{gridColumn:'1 / -1'}} className="food-search-block">
          <div className="row gap" style={{marginBottom:6}}>
            ...
            <button type="button" className="btn" onClick={runRemoteSearch} disabled={remoteLoading || !nutritionApiEnabled} title={!nutritionApiEnabled ? 'Remote disabled in Settings' : 'Refresh remote'}>
              {remoteLoading? '...' : (!nutritionApiEnabled ? 'Off' : '↻')}
            </button>
          </div>
          {(foodResults.length>0 || remoteResults.length>0 || remoteLoading || remoteError) && (
