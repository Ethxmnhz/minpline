...
export const QuickFoodAdd: React.FC<QuickFoodAddProps> = ({ onClose, date }) => {
  ...
  useEffect(()=>{
    ...
    if (!q) { setLocalResults([]); setRemoteResults([]); if (remoteError) setRemoteError(''); return; }
    setLocalResults(searchFoods(q));
    if (!nutritionApiEnabled) {
      setRemoteResults([]); setRemoteLoading(false);
      if (q.length>=2) setRemoteError('Remote disabled (Settings)'); else if (remoteError) setRemoteError('');
      return () => { active=false; };
    }
  ...
  }, [localQuery, nutritionApiEnabled]);
      ...
      <div className="quick-add-sheet">
        ...
        <div className="qa-section">
          ...
          <div className="row gap" style={{marginBottom:6}}>
            ...
            <button type="button" className="btn" onClick={runRemote} disabled={remoteLoading || !nutritionApiEnabled} title={!nutritionApiEnabled ? 'Remote disabled in Settings' : 'Refresh remote'}>
