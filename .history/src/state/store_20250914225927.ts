...
export interface TrackerState {
  ...
  nutritionCache: Record<string, { ts: number; items: any[] }>;
  apiStats: { nutritionCalls: number; cacheHits: number };
  ...
  cacheNutrition: (query: string, items: any[]) => void;
  getCachedNutrition: (query: string) => any[] | null;
  incNutritionCall: () => void;
  incNutritionCacheHit: () => void;
}
...
export const useStore = create<TrackerState>()(persist((set, get) => ({
  ...
  workoutSets: {},
  nutritionCache: {},
  apiStats: { nutritionCalls: 0, cacheHits: 0 },
  cloudEnabled: false,
  ...
  cacheNutrition: (query, items) => set(s => ({ nutritionCache: { ...s.nutritionCache, [query]: { ts: Date.now(), items } } })),
  getCachedNutrition: (query) => {
    const entry = get().nutritionCache[query];
    if (!entry) return null;
    // TTL 6 hours
    if (Date.now() - entry.ts > 6 * 60 * 60 * 1000) return null;
    return entry.items;
  },
  incNutritionCall: () => set(s => ({ apiStats: { ...s.apiStats, nutritionCalls: s.apiStats.nutritionCalls + 1 } })),
  incNutritionCacheHit: () => set(s => ({ apiStats: { ...s.apiStats, cacheHits: s.apiStats.cacheHits + 1 } })),
  exportData: () => {
    const s = get();
  ...
  importData: (data: any) => {
    ...
    set(s => ({
      ...
      nutritionCache: data.nutritionCache || s.nutritionCache,
      apiStats: data.apiStats || s.apiStats
    }));
  }
