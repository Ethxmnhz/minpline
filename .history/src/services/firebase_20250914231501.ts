...
export async function ensureAnonAuth(): Promise<string | null> {
  ...
  try {
    ...
    console.warn('Anonymous auth failed', e?.code || e?.message || e);
    return null;
  }
...
// Global (public) data path (no per-user segregation). WARNING: all clients share.
const GLOBAL_PATH = 'global/trackerData';
...
export async function saveGlobalData(data: any) {
  await set(ref(db, GLOBAL_PATH), data);
}

export function subscribeGlobalData(cb: (data: any)=>void) {
  const r = ref(db, GLOBAL_PATH);
  return onValue(r, snap => cb(snap.val()));
}
